...
const SearchMeal = ({ navigation, route }) => {
  ...
  const getapithucdon = async () => {
    try {
      ...
      const response = await axios.get('http://192.168.133.46:3000/api/getAllDish');
  ...
  const firstFoodmealType = getFirstFoodProcessingType(filteredData);
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.content}>
        <View>
          <Text style={styles.textHeadList}>Loại bữa ăn: {firstFoodmealType}</Text>
